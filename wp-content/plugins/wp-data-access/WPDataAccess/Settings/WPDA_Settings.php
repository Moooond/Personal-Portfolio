...
class WPDA_Settings
{
    ...
    const  DROPBOX_CLIENT_ID = 'f6e7znb7qfwaqjh' ;
    // 'rv5japeynhpzmyy';
    const  DROPBOX_CLIENT_SECRET = '0vzaidexrtcede4' ;
    // 'v45glikrzr6h62z';
    ...
    public function __construct()
    {
        ...
        if ( isset( $_REQUEST['tab'] ) ) {
            ...
            if ( isset( $this->tabs[$_REQUEST['tab']] ) ) {
                ...
                $this->current_tab = sanitize_text_field( wp_unslash( $_REQUEST['tab'] ) );
                // input var okay.
            }
        ...
        if ( 'repository' === $this->current_tab && isset( $_REQUEST['repos'] ) && 'true' === sanitize_text_field( wp_unslash( $_REQUEST['repos'] ) ) ) {
            // input var okay.
            // Security check.
            ...
            WPDA::set_option( WPDA::OPTION_WPDA_SETUP_ERROR );
            // Set to default.
            WPDA::set_option( WPDA::OPTION_WPDA_SHOW_WHATS_NEW );
            // Set to default.
    ...
    protected function add_content_system()
    {
        ...
        $query = "select table_name AS table_name, engine AS engine, table_collation AS table_collation " . "from information_schema.tables " . "where table_schema = '{$wpdb->dbname}' " . "and table_name in " . "('{$menus_table_name}', '{$design_table_name}', '{$logging_table_name}', " . "'{$data_projects_project_name}', '{$data_projects_page_name}', '{$data_projects_table_name}', " . "'{$media_table_name}', '{$data_publication_table_name}', '{$table_settings_table_name}')";
			...
			<script type='text/javascript'>
				jQuery(function () {
					...
					text_to_clipboard.on('success', function (e) {
						jQuery.notify('<?php 
        echo  __( 'System info successfully copied to clipboard!' ) ;
        ?>','info');
					});
					text_to_clipboard.on('error', function (e) {
						jQuery.notify('<?php 
        echo  __( 'Could not copy system info to clipboard!' ) ;
        ?>','error');
					});
				});
			</script>
        ...
        ?>
				...
				<tr>
					<th class="wpda_system_info_title"><?php 
        echo  __( 'Database Management System' ) ;
        ?></th>
					<td>
						<table class="wpda-table-system-info" style="width:100%">
							<tr>
        ...
        ?>
							...
							<tr>
								<th class="wpda_system_info_subtitle"><?php 
        echo  __( 'Database Character Set' ) ;
        ?></th>
								<td class="wpda_system_info_value" colspan="2">
									<?php 
        echo  DB_CHARSET ;
        ...
        ?>
							...
							<tr>
								<th class="wpda_system_info_subtitle"><?php 
        echo  __( 'WP Debugging Mode' ) ;
        ?></th>
								<td class="wpda_system_info_value" colspan="2">
									<?php 
        echo  ( !defined( 'WP_DEBUG' ) ? 'undefined' : (( true === WP_DEBUG ? 'true' : 'false' )) ) ;
        ?>
								</td>
							</tr>
						</table>
        ...
        ?></th>
								<td class="wpda_system_info_value" colspan="2">
									<?php 
        echo  WPDA::get_option( WPDA::OPTION_WPDA_VERSION ) ;
        ?>
								</td>
							</tr>
							<tr>
        ...
        ?>
					...
					<tr>
						...
						<td>
							<span class="dashicons dashicons-yes"></span>
							<?php 
        echo  __( 'jQuery DataTables (+Responsive) is needed in the Front-end to support publications on your website', 'wp-data-access' ) ;
        ?>
							<br/>
							<span class="dashicons dashicons-yes"></span>
							<?php 
        echo  __( 'jQuery DataTables (+Responsive) is needed in the Back-end to test publications in the WordPress dashboard', 'wp-data-access' ) ;
        ?>
							<br/>
							<span class="dashicons dashicons-yes"></span>
							<?php 
    ...
    protected function add_content_datapublisher()
    {
        ...
        ?>&tab=datapublisher">
				<table class="wpda-table-settings">
					<tr>
						<th><?php 
        echo  __( 'Publication style', 'wp-data-access' ) ;
        ?></th>
						<td>
							<select name="publication_style">
								<option value="default" <?php 
        echo  ( 'default' === $publication_style ? 'selected' : '' ) ;
        ?>>Default</option>
								<option value="compact" <?php 
        echo  ( 'compact' === $publication_style ? 'selected' : '' ) ;
        ?>>Compact</option>
								<option value="jqueryui" <?php 
        echo  ( 'jqueryui' === $publication_style ? 'selected' : '' ) ;
        ?>>jQuery UI</option>
								<option value="semantic" <?php 
        echo  ( 'semantic' === $publication_style ? 'selected' : '' ) ;
        ?>>Semantic</option>
								<option value="foundation" <?php 
        echo  ( 'foundation' === $publication_style ? 'selected' : '' ) ;
        ?>>Foundation</option>
								<option value="bootstrap" <?php 
        echo  ( 'bootstrap' === $publication_style ? 'selected' : '' ) ;
        ?>>Bootstrap 3</option>
								<option value="bootstrap4" <?php 
        echo  ( 'bootstrap4' === $publication_style ? 'selected' : '' ) ;
        ?>>Bootstrap 4</option>
							</select>
						</td>
					</tr>
					<tr>
						<th><?php 
        echo  __( 'Default jQuery UI theme', 'wp-data-access' ) ;
        ?></th>
						<td>
							<a href="options-general.php?page=wpdataaccess&tab=frontend">
        ...
        ?>
					...
					<tr>
						<th><?php 
        echo  __( 'Data Publisher Tool Access', 'wp-data-access' ) ;
        ?></th>
    ...
    protected function add_content_dataforms()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'save' === $action ) {
                WPDA::set_option( WPDA::OPTION_PLUGIN_WPDADATAFORMS_ALLOW_ANONYMOUS_ACCESS, ( isset( $_REQUEST['allow_anonymous_access'] ) ? 'on' : 'off' ) );
            } elseif ( 'setdefaults' === $action ) {
                // Set all publication settings back to default.
        ...
        ?>&tab=dataforms">
				<table class="wpda-table-settings">
					<tr>
						<th><?php 
        echo  __( 'Allow anonymous access', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox" name="allow_anonymous_access" <?php 
        echo  ( 'on' === $allow_anonymous_access ? 'checked' : '' ) ;
        ?>/>Enabled
							</label>
						</td>
					</tr>
        ...
        ?></th>
					...
					<tr>
						...
						<td>
							<span class="dashicons dashicons-yes"></span>
							<?php 
        echo  __( 'Data Forms are styled according to the default jQuery UI theme', 'wp-data-access' ) ;
        ?>
							<br/>
							<span class="dashicons dashicons-yes"></span>
							<?php 
        echo  __( 'Individual styling per shortcode will be added later', 'wp-data-access' ) ;
        ?>
						</td>
					</tr>
				</table>
				<div class="wpda-table-settings-button">
					...
					<input type="submit"
						   value="<?php 
        echo  __( 'Save Data Forms  Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button">
						<?php 
        echo  __( 'Reset Data Forms Settings To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-forms-settings-' . WPDA::get_current_user_login(), '_wpnonce', false );
        ?>
			</form>
			<?php 
    }
    ...
    protected function add_content_databackup()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'save' === $action ) {
                // Save options.
                $save_local_path = ( isset( $_REQUEST['local_path'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['local_path'] ) ) : '' );
                // input var okay.
                ...
                if ( '' !== $dropbox_auth && $dropbox_auth_saved !== $dropbox_auth ) {
                    ...
                    if ( !(200 === $response->getStatusCode() && 'OK' === $response->getReasonPhrase()) ) {
                        $msg = new WPDA_Message_Box( [
                            'message_text'           => __( 'Dropbox authorization failed ', 'wp-data-access' ) . $response->getStatusCode() . ' ' . $response->getReasonPhrase(),
                            'message_type'           => 'error',
                            'message_is_dismissible' => false,
                        ] );
                        $msg->box();
        ...
        $file_permission = fileperms( $local_path );
        error_reporting( $error_level );
        $owner_info = ( $file_permission & 0x100 ? 'r' : '-' );
        $owner_info .= ( $file_permission & 0x80 ? 'w' : '-' );
        $owner_info .= ( $file_permission & 0x40 ? ( $file_permission & 0x800 ? 's' : 'x' ) : (( $file_permission & 0x800 ? 'S' : '-' )) );
        $group_info = ( $file_permission & 0x20 ? 'r' : '-' );
        $group_info .= ( $file_permission & 0x10 ? 'w' : '-' );
        $group_info .= ( $file_permission & 0x8 ? ( $file_permission & 0x400 ? 's' : 'x' ) : (( $file_permission & 0x400 ? 'S' : '-' )) );
        $world_info = ( $file_permission & 0x4 ? 'r' : '-' );
        $world_info .= ( $file_permission & 0x2 ? 'w' : '-' );
        $world_info .= ( $file_permission & 0x1 ? ( $file_permission & 0x200 ? 't' : 'x' ) : (( $file_permission & 0x200 ? 'T' : '-' )) );
        ...
        ?>&tab=databackup">
				<table class="wpda-table-settings">
					<tr>
						<th><?php 
        echo  __( 'Local file system' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox"
									   name="local_path_activated" <?php 
        if ( isset( $options_activated['local_path'] ) ) {
            echo  'checked' ;
        }
        ?> />
								<?php 
        echo  __( 'Activated', 'wp-data-access' ) ;
        ?>
							</label>
							<br/><br/>
							<?php 
        echo  __( 'Enter the name of the folder where data backup files should be stored.' ) ;
        ?>
							<br/>
							<input type="text" name="local_path" value="<?php 
        echo  $local_path ;
        ?>"/>
							<span><?php 
        echo  __( 'Make sure the folder exists with permission to write files.' ) ;
        ?></span>
							<?php 
        if ( 'WIN' !== strtoupper( substr( PHP_OS, 0, 3 ) ) ) {
            ...
            if ( !$file_permission ) {
                ...
                if ( '4' !== substr( decoct( $file_permission ), 0, 1 ) ) {
                    ...
                    echo  __( '"name"' ) ;
                    ?>: "<?php 
                    echo  posix_getpwuid( $fileowner )['name'] ;
                    ?>",
										<?php 
                    echo  __( '"access"' ) ;
                    ?>: "<?php 
                    ...
                    echo  __( '"name"' ) ;
                    ?>: "<?php 
                    echo  posix_getpwuid( $groupowner )['name'] ;
                    ?>",
										<?php 
                    echo  __( '"access"' ) ;
                    ?>: "<?php 
        ...
        ?>
							...
							<a href="https://www.dropbox.com/" class="button button-secondary" target="_blank">
								<?php 
        echo  __( 'Create a Dropbox account' ) ;
        ?>
							</a>
							<span style="vertical-align:-webkit-baseline-middle;">
								<?php 
        echo  __( 'You can skip this step if you already have an account.' ) ;
        ?>
							</span>
							<br/><br/>
							<?php 
        echo  __( 'Authorize the WP Data Access Dropbox app and enter the authorization code in the text box below.' ) ;
        ?>
							<br/>
							<input type="text" name="dropbox_auth" value="<?php 
        echo  $dropbox_auth ;
        ?>"/>
							<a href="https://www.dropbox.com/oauth2/authorize?response_type=code&client_id=<?php 
        echo  self::DROPBOX_CLIENT_ID ;
        ?>"
							   class="button button-secondary"
							   target="_blank"
							   style="vertical-align:bottom;">
								<?php 
        echo  __( 'Get Dropbox authorization code' ) ;
        ?>
							</a>
							<br/><br/>
							<?php 
        echo  __( 'Enter the name of the folder where data backup files should be stored. If the folder doesn\'t exists, it\'ll be created for you.' ) ;
        ?>
							<br/>
							<input type="text" name="dropbox_folder" value="<?php 
        echo  $dropbox_folder ;
        ?>"/>
						</td>
					</tr>
				...
				<div class="wpda-table-settings-button">
					...
					<input type="submit"
						   value="<?php 
        echo  __( 'Save Data Backup Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button">
						<?php 
        echo  __( 'Reset Data Backup To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-databackup-settings-' . WPDA::get_current_user_login(), '_wpnonce', false );
        ?>
			</form>
    ...
    protected function add_content_uninstall()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'save' === $action ) {
                // Save changes.
                WPDA::set_option( WPDA::OPTION_WPDA_UNINSTALL_TABLES, ( isset( $_REQUEST['delete_tables'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['delete_tables'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_WPDA_UNINSTALL_OPTIONS, ( isset( $_REQUEST['delete_options'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['delete_options'] ) ) : 'off' ) );
            } elseif ( 'setdefaults' === $action ) {
        ...
        ?>&tab=uninstall">
				<table class="wpda-table-settings">
					<tr>
						<th>
							<?php 
        echo  __( 'On Plugin Uninstall', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<label>
								<input type="checkbox" name="delete_plugin" style="margin-right: 0" checked
									   disabled="disabled">
								<?php 
        echo  __( 'Delete plugin', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
        ...
        echo  ( 'on' === $delete_tables ? 'checked' : '' ) ;
        ?>>
								<?php 
        echo  __( 'Delete plugin tables (all data will be lost)', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="delete_options"
									   style="margin-right: 0" <?php 
        echo  ( 'on' === $delete_options ? 'checked' : '' ) ;
        ?>>
								<?php 
        echo  __( 'Delete plugin settings (all settings will be lost)', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
				...
				<div class="wpda-table-settings-button">
					...
					<input type="submit"
						   value="<?php 
        echo  __( 'Save Uninstall Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button button-secondary">
						<?php 
        echo  __( 'Reset Uninstall Settings To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-uninstall-settings-' . WPDA::get_current_user_login(), '_wpnonce', false );
        ?>
			</form>
    ...
    protected function add_content_roles()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'delete' === $_REQUEST['action'] ) {
                $wp_nonce = ( isset( $_REQUEST['_wpnoncedelrole'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['_wpnoncedelrole'] ) ) : '' );
                // input var okay.
            ...
            if ( 'save' === $_REQUEST['action'] ) {
                WPDA::set_option( WPDA::OPTION_WPDA_ENABLE_ROLE_MANAGEMENT, ( isset( $_REQUEST['enable_role_management'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['enable_role_management'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_WPDA_USE_ROLES_IN_SHORTCODE, ( isset( $_REQUEST['use_roles_in_shortcode'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['use_roles_in_shortcode'] ) ) : 'off' ) );
                
                if ( isset( $_REQUEST['wpda_role_name'] ) && is_array( $_REQUEST['wpda_role_name'] ) && isset( $_REQUEST['wpda_role_label'] ) && is_array( $_REQUEST['wpda_role_label'] ) ) {
                    $no_roles = count( $_REQUEST['wpda_role_name'] );
                    for ( $i = 0 ;  $i < $no_roles ;  $i++ ) {
                        $sanitized_new_role_name = sanitize_text_field( wp_unslash( $_REQUEST['wpda_role_name'][$i] ) );
                        // input var okay.
                        $sanitized_new_role_label = sanitize_text_field( wp_unslash( $_REQUEST['wpda_role_label'][$i] ) );
                        // input var okay.
                ...
                if ( isset( $_REQUEST['delete_role_name'] ) ) {
                    $sanitized_role_name = sanitize_text_field( wp_unslash( $_REQUEST['delete_role_name'] ) );
                    // input var okay.
                    $all_users = get_users( [
        ...
        ?>&tab=roles">
				...
				<table class="wpda-table-settings">
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Plugin Role Management', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<label>
								<input type="checkbox" name="enable_role_management"
									<?php 
        echo  ( 'on' === $enable_role_management ? 'checked' : '' ) ;
        ?>/>
								<?php 
        echo  __( 'Enable role management', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="use_roles_in_shortcode"
									<?php 
        echo  ( 'on' === $use_roles_in_shortcode ? 'checked' : '' ) ;
        ?>/>
								<?php 
        echo  __( 'Use roles in Data Projects shortcodes', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Available Roles', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<div id="list_roles">
        ...
        foreach ( $wp_roles->roles as $role => $val ) {
            ...
            echo  ( $is_wp_role ? 'dashicons-wordpress' : 'dashicons-trash' ) ;
            ?>"
									  style="font-size: 14px; vertical-align: text-top;<?php 
            echo  ( $is_wp_role ? '' : ' cursor: pointer;' ) ;
            ?>"></span>
										<?php 
            echo  $role_label ;
            ?>
        ...
        ?>
				...
				<div class="wpda-table-settings-button">
					...
					<input type="submit"
						   value="<?php 
        echo  __( 'Save Manage Roles Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button button-secondary">
						<?php 
        echo  __( 'Reset Manage Roles Settings To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-manage-roles-settings-' . WPDA::get_current_user_login(), '_wpnonce', false );
        ?>
        ...
        ?>&tab=roles">
				...
				<?php 
        wp_nonce_field( 'wpda-manage-roles-settings-' . WPDA::get_current_user_login(), '_wpnoncedelrole', false );
        ?>
			</form>
    ...
    protected function count_repository_backup_table( $table_name )
    {
        ...
        $query_table_name = str_replace( '`', '', $table_name );
    ...
    protected function add_content_repository()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'save' === $action ) {
                // Save changes.
                WPDA::set_option( WPDA::OPTION_MR_KEEP_BACKUP_TABLES, ( isset( $_REQUEST['keep_backup_tables'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['keep_backup_tables'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_MR_BACKUP_TABLES_KEPT, ( isset( $_REQUEST['backup_tables_kept'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['backup_tables_kept'] ) ) : WPDA::OPTION_MR_BACKUP_TABLES_KEPT_DEFAULT ) );
            } elseif ( 'setdefaults' === $action ) {
                // Set back to default values.
        ...
        $query = "select table_name AS table_name from information_schema.tables " . "where table_schema = '{$wpdb->dbname}' " . " and ( table_name like '{$menus_table_name}{$bck_postfix}%' " . " or table_name like '{$design_table_name}{$bck_postfix}%' " . " or table_name like '{$table_settings_table_name}{$bck_postfix}%' " . " or table_name like '{$logging_table_name}{$bck_postfix}%' " . " or table_name like '{$media_table_name}{$bck_postfix}%' " . " or table_name like '{$data_publication_table_name}{$bck_postfix}%' " . " or table_name like '{$csv_import_table_name}{$bck_postfix}%' " . " or table_name like '{$data_projects_project_name}{$bck_postfix}%' " . " or table_name like '{$data_projects_page_name}{$bck_postfix}%' " . " or table_name like '{$data_projects_table_name}{$bck_postfix}%' )" . "order by 1 desc";
        ...
        if ( isset( $_REQUEST['create_backup'] ) && 'true' === $_REQUEST['create_backup'] ) {
            ...
            if ( isset( $_REQUEST['backup_date'] ) ) {
                $backup_date = sanitize_text_field( wp_unslash( $_REQUEST['backup_date'] ) );
                // input var okay.
            ...
            foreach ( $backup_tables as $backup_table ) {
                $drop_table = str_replace( '`', '', "{$backup_table[0]}" );
                // prevent SQL injection
            ...
            if ( isset( $_REQUEST['restore_date'] ) ) {
                ...
                $restore_date = sanitize_text_field( wp_unslash( $_REQUEST['restore_date'] ) );
                // input var okay.
        ...
        ?>
			<form id="wpda-download-actual-respository"
				  method="post"
				  action="admin.php?action=wpda_export&type=table&_wpnonce=<?php 
        echo  esc_attr( $wpnonce_download_backup ) ;
        ?>&show_create=off"
				  target="_blank"
				  style="display: none"
			>
				<input type="hidden" name="table_names[]" value="<?php 
        echo  esc_attr( $table_settings_table_name ) ;
        ?>">
				<input type="hidden" name="table_names[]" value="<?php 
        echo  esc_attr( $media_table_name ) ;
        ...
        ?>">
				<input type="hidden" name="table_names[]" value="<?php 
        echo  esc_attr( $csv_import_table_name ) ;
        ?>">
				<input type="hidden" name="table_names[]" value="<?php 
        echo  esc_attr( $logging_table_name ) ;
        ?>">
        ...
        echo  esc_attr( $wpnonce_remove_backup ) ;
        ?>">
				<input type="submit">
			</form>
        ...
        ?>&tab=repository"
			...
			>
				...
				<input type="hidden" name="_wpnonce" value="<?php 
        echo  esc_attr( $wpnonce_create_backup ) ;
        ?>">
				<input type="submit">
			</form>
        ...
        ?>&tab=repository"
			...
			>
				...
				<input type="hidden" name="_wpnonce" value="<?php 
        echo  esc_attr( $wpnonce_restore_backup ) ;
        ?>">
				<input type="submit">
			</form>
        ...
        ?>&tab=repository">
				<table class="wpda-table-settings">
					...
					<tr>
						<th>
							<?php 
        echo  __( 'On Plugin Update', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<label>
								<input type="checkbox" name="keep_backup_tables"
									   <?php 
        echo  ( 'on' === $keep_backup_tables ? 'checked' : '' ) ;
        ?>>
								<strong><?php 
        echo  __( 'Keep backup of repository tables', 'wp-data-access' ) ;
        ?></strong>
								<?php 
        echo  __( '(creates backup tables on plugin updates)', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label style="display: inline-block; margin-top: 5px">
								<strong>Max backups saved:</strong>&nbsp;
								<input type="number" name="backup_tables_kept" value="<?php 
        echo  esc_attr( $backup_tables_kept ) ;
        ?>" />
							</label>
							<br/><br/>
							<a href="javascript:void(0)"
							...
							>
								<?php 
        echo  __( 'Download actual repository tables' ) ;
        ?>
							</a>
							<?php 
        ...
        if ( $no_backup_tables > 0 ) {
            ...
            foreach ( $backup_tables as $backup_table ) {
                ...
                if ( strtolower( "{$data_projects_page_name}{$bck_postfix}" ) === strtolower( substr( $backup_table[0], 0, strlen( "{$data_projects_page_name}{$bck_postfix}" ) ) ) ) {
                    $backup_date = substr( $backup_table[0], 25 + strlen( $wpdb->prefix ) );
                    $display_date = substr( $backup_date, 0, 4 ) . '-' . substr( $backup_date, 4, 2 ) . '-' . substr( $backup_date, 6, 2 ) . ' ' . substr( $backup_date, 8, 2 ) . ':' . substr( $backup_date, 10, 2 ) . ':' . substr( $backup_date, 12, 2 );
                    ?>
										<tr data-backup-date="<?php 
                    echo  esc_attr( $backup_date ) ;
                    ?>">
											<td>
												<span>
													Repository backup <?php 
                    ...
                    ?>
											...
											<td>
												<span class="dashicons dashicons-download wpda_tooltip"
													  title="Download backup: <?php 
                    echo  esc_attr( $display_date ) ;
                    ?>"
													  onclick="backup_respository_tables('<?php 
                    echo  "{$table_settings_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$media_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$design_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$data_projects_project_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$data_projects_page_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$data_projects_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$data_publication_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$menus_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$csv_import_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>', '<?php 
                    echo  "{$logging_table_name}{$bck_postfix}{$backup_date}" ;
                    ?>')"
												>
												</span>
											</td>
                    ...
                    ?>"
											...
											<td>
												<span class="dashicons dashicons-trash wpda_tooltip"
													  title="Remove repository backup: <?php 
                    echo  esc_attr( $display_date ) ;
                    ?>"
													  onclick="if (confirm('Remove backup?')) { jQuery('#remove_backup_date').val('<?php 
                    echo  esc_attr( $backup_date ) ;
                    ?>'); jQuery('#wpda-remove-backup').submit(); }"
												>
												</span>
											</td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $table_settings_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $media_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $design_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $data_projects_project_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $data_projects_page_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $data_projects_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $data_publication_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $menus_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $csv_import_table_name . $bck_postfix . $backup_date ) ;
                    ?></td></td>
											<td class="wpda-repository-column"><?php 
                    echo  $this->count_repository_backup_table( $logging_table_name . $bck_postfix . $backup_date ) ;
                    ?></td>
										</tr>
										<?php 
                }
        ...
        ?>
							...
							<a href="javascript:void(0)"
							   class="button"
							   onclick=" if (confirm('<?php 
        echo  __( 'Backup repository tables?', 'wp-data-access' ) ;
        ?>')) { jQuery('#wpda-create-backup').submit(); }"
							>
								<?php 
        echo  __( 'Create new repository backup' ) ;
        ?>
							</a>
							<a href="javascript:void(0)"
							   class="button <?php 
        echo  ( 0 === $no_backup_tables ? 'disabled' : '' ) ;
        ?>"
							   onclick="if (confirm('<?php 
        echo  __( 'Delete all backup tables?\\nThis action cannot be undone.\\n\\\'Cancel\\\' to stop, \\\'OK\\\' to delete.', 'wp-data-access' ) ;
        ?>')) { jQuery('#wpda-remove-all-backups').submit(); }"
							>
								<?php 
        echo  __( 'Delete all (' ) . esc_html( $no_backup_tables ) . __( ') repository backup tables' ) ;
        ?>
							</a>
						</td>
					</tr>
        ...
        ?>
						...
						<td>
							<span class="dashicons <?php 
        echo  ( $table_settings_table_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $table_settings_table_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $table_settings_table_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_table_settings ) ;
            ?>
									<?php 
            echo  __( 'table settings defined in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Manage Media', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $media_table_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $media_table_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $media_table_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_media ) ;
            ?>
									<?php 
            echo  __( 'media columns defined in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Data Designer', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $design_table_name_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $design_table_name_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $design_table_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_table_designs ) ;
            ?>
									<?php 
            echo  __( 'table designs in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Data Projects', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        ...
        if ( $data_projects_project_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_projects ) ;
            ?>
									<?php 
            echo  __( 'data projects in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        if ( $data_projects_page_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_pages ) ;
            ?>
									<?php 
            echo  __( 'project pages in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        if ( $data_projects_table_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_project_table_designs ) ;
            ?>
									<?php 
            echo  __( 'project tables in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Data Publisher', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $data_publication_table_name_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $data_publication_table_name_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $data_publication_table_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_data_publication ) ;
            ?>
									<?php 
            echo  __( 'publication in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Data Menus', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $menus_table_name_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $menus_table_name_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $menus_table_name_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_menu_items ) ;
            ?>
									<?php 
            echo  __( 'menus in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'CSV Uploads', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $csv_import_table_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $csv_import_table_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        ?>
					...
					<tr>
						<th>
							<?php 
        echo  __( 'Data Logging', 'wp-data-access' ) ;
        ?>
						</th>
						<td>
							<span class="dashicons <?php 
        echo  ( $logging_table_exists ? 'dashicons-yes' : 'dashicons-no' ) ;
        ?>"></span>
							<?php 
        echo  esc_attr( $table ) ;
        ?>
        ...
        ?></strong>
							<?php 
        echo  ( $logging_table_exists ? esc_attr( $found ) : esc_attr( $not_found ) ) ;
        ?>
							<?php 
        ...
        if ( $logging_table_exists ) {
            ?>
								...
								<strong>
									<?php 
            echo  esc_attr( $no_logs ) ;
            ?>
									<?php 
            echo  __( 'logging rows in repository', 'wp-data-access' ) ;
            ?>
								</strong>
								<?php 
        }
        ...
        ?>
				...
				<div class="wpda-table-settings-button">
					...
					<input type="submit"
						   value="<?php 
        echo  __( 'Save Manage Respository Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button button-secondary">
						<?php 
        echo  __( 'Reset Manage Repository Settings To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
					<?php 
        ...
        ?>
					<a href="?page=<?php 
        echo  esc_attr( $this->page ) ;
        ?>&tab=repository&repos=true&_wpnonce=<?php 
        echo  esc_attr( $wpnonce_recreate ) ;
        ?>"
					   class="button button-secondary">
						<?php 
        echo  __( 'Recreate', 'wp-data-access' ) ;
        ?> WP Data Access
						<?php 
        echo  __( 'Repository', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-repository-settings-' . WPDA::get_current_user_login(), '_wpnonce', false );
        ?>
			</form>
        ...
        if ( !$menus_table_name_exists || !$design_table_name_exists || !$data_projects_project_name_exists || !$data_projects_page_name_exists || !$data_projects_table_name_exists || !$data_publication_table_name_exists ) {
            ?>
					<p><strong><?php 
            echo  __( 'Your repository has errors!', 'wp-data-access' ) ;
            ?></strong></p>
					<p>
						<?php 
            echo  __( 'Recreate the WP Data Access repository to solve this problem.', 'wp-data-access' ) ;
            ?>
						<?php 
            echo  __( 'Please leave your comments on the support forum if the problem remains.', 'wp-data-access' ) ;
            ?>
    ...
    protected function add_content_backend()
    {
        ...
        if ( isset( $_REQUEST['action'] ) ) {
            ...
            if ( 'save' === $action ) {
                ...
                if ( $is_wp_database ) {
                    WPDA::set_option( WPDA::OPTION_BE_TABLE_ACCESS, ( isset( $_REQUEST['table_access'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['table_access'] ) ) : null ) );
                } else {
                    update_option( WPDA::BACKEND_OPTIONNAME_DATABASE_ACCESS . $database, ( isset( $_REQUEST['table_access'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['table_access'] ) ) : null ) );
                }
                ...
                if ( $is_wp_database ) {
                    WPDA::set_option( WPDA::OPTION_BE_TABLE_ACCESS_SELECTED, $table_access_selected_new_value_checked );
                } else {
                    update_option( WPDA::BACKEND_OPTIONNAME_DATABASE_SELECTED . $database, $table_access_selected_new_value_checked );
                }
                
                WPDA::set_option( WPDA::OPTION_BE_VIEW_LINK, ( isset( $_REQUEST['view_link'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['view_link'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_ALLOW_INSERT, ( isset( $_REQUEST['allow_insert'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['allow_insert'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_ALLOW_UPDATE, ( isset( $_REQUEST['allow_update'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['allow_update'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_ALLOW_DELETE, ( isset( $_REQUEST['allow_delete'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['allow_delete'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_EXPORT_ROWS, ( isset( $_REQUEST['export_rows'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['export_rows'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_EXPORT_VARIABLE_PREFIX, ( isset( $_REQUEST['export_variable_rows'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['export_variable_rows'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_ALLOW_IMPORTS, ( isset( $_REQUEST['allow_imports'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['allow_imports'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_CONFIRM_EXPORT, ( isset( $_REQUEST['confirm_export'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['confirm_export'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_CONFIRM_VIEW, ( isset( $_REQUEST['confirm_view'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['confirm_view'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_PAGINATION, ( isset( $_REQUEST['pagination'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['pagination'] ) ) : null ) );
                WPDA::set_option( WPDA::OPTION_BE_REMEMBER_SEARCH, ( isset( $_REQUEST['remember_search'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['remember_search'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_INNODB_COUNT, ( isset( $_REQUEST['innodb_count'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['innodb_count'] ) ) : 100000 ) );
                WPDA::set_option( WPDA::OPTION_BE_DESIGN_MODE, ( isset( $_REQUEST['design_mode'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['design_mode'] ) ) : 'basic' ) );
                WPDA::set_option( WPDA::OPTION_BE_TEXT_WRAP_SWITCH, ( isset( $_REQUEST['text_wrap_switch'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['text_wrap_switch'] ) ) : 'off' ) );
                WPDA::set_option( WPDA::OPTION_BE_TEXT_WRAP, ( isset( $_REQUEST['text_wrap'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['text_wrap'] ) ) : 400 ) );
                
                if ( isset( $_REQUEST['wpda_default_user'] ) && isset( $_REQUEST['wpda_default_database'] ) && '' !== $_REQUEST['wpda_default_user'] && '' !== $_REQUEST['wpda_default_database'] ) {
                    $default_databases = get_option( 'wpda_default_database' );
                    if ( false === $default_databases ) {
                        $default_databases = [];
                    }
                    $wpda_default_user = sanitize_text_field( wp_unslash( $_REQUEST['wpda_default_user'] ) );
                    // input var okay.
                    $wpda_default_database = sanitize_text_field( wp_unslash( $_REQUEST['wpda_default_database'] ) );
                    // input var okay.
                ...
                if ( $is_wp_database ) {
                    ...
                    update_option( WPDA::BACKEND_OPTIONNAME_DATABASE_ACCESS . $database, 'show' );
                    update_option( WPDA::BACKEND_OPTIONNAME_DATABASE_SELECTED . $database, '' );
                }
                ...
                if ( isset( $_REQUEST['wpda_default_database_delete'] ) ) {
                    $delete_user_id = sanitize_text_field( wp_unslash( $_REQUEST['wpda_default_database_delete'] ) );
                    // input var okay.
                    $default_databases = get_option( 'wpda_default_database' );
							...
							<script type='text/javascript'>
								jQuery(function () {
									...
									jQuery('#schema_name').on('change', function() {
										window.location = '?page=<?php 
        echo  esc_attr( $this->page ) ;
        ?>&tab=backend&database=' + jQuery(this).val();
									});
								});
							</script>
						</td>
					...
					<tr>
						<th><?php 
        echo  __( 'Row access', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input
										...
										name="view_link"
									<?php 
        echo  ( 'on' === $view_link ? 'checked' : '' ) ;
        ?>
								><?php 
        echo  __( 'Add view link to list table', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php 
        echo  __( 'Allow transactions?', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox" name="allow_insert"
									<?php 
        echo  ( 'on' === $allow_insert ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Allow insert', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="allow_update"
									<?php 
        echo  ( 'on' === $allow_update ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Allow update', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="allow_delete"
									<?php 
        echo  ( 'on' === $allow_delete ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Allow delete', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php 
        echo  __( 'Allow exports?', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox" name="export_rows"
									<?php 
        echo  ( 'on' === $export_rows ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Allow row export', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="export_variable_rows"
									<?php 
        echo  ( 'on' === $export_variable_rows ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Export with variable WP prefix', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php 
        echo  __( 'Allow imports?', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox" name="allow_imports"
									<?php 
        echo  ( 'on' === $allow_imports ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'Allow to import scripts from Data Explorer table pages', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
					<tr>
						<th><?php 
        echo  __( 'Ask for confirmation?', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input type="checkbox" name="confirm_export"
									<?php 
        echo  ( 'on' === $confirm_export ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'When starting export', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<label>
								<input type="checkbox" name="confirm_view"
									<?php 
        echo  ( 'on' === $confirm_view ? 'checked' : '' ) ;
        ?> /><?php 
        echo  __( 'When viewing non WPDA table', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
        ...
        ?>">
					...
					<tr>
						<th><?php 
        echo  __( 'Search box', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input
										type="checkbox"
										name="remember_search" <?php 
        echo  ( 'on' === $remember_search ? 'checked' : '' ) ;
        ?>
								><?php 
        echo  __( 'Remember last search', 'wp-data-access' ) ;
        ?>
							</label>
						</td>
					</tr>
        ...
        ?>">
					...
					<tr>
						<th><?php 
        echo  __( 'Default designer mode', 'wp-data-access' ) ;
        ?></th>
						<td>
							<select name="design_mode">
								<option value="basic" <?php 
        echo  ( 'basic' === $design_mode ? 'selected' : '' ) ;
        ?>>Basic
								</option>
								<option value="advanced" <?php 
        echo  ( 'advanced' === $design_mode ? 'selected' : '' ) ;
        ?>>
									Advanced
								</option>
							</select>
					...
					<tr>
						<th><?php 
        echo  __( 'Content wrap', 'wp-data-access' ) ;
        ?></th>
						<td>
							<label>
								<input
										type="checkbox"
										name="text_wrap_switch" <?php 
        echo  ( 'on' === $text_wrap_switch ? 'checked' : '' ) ;
        ?>
								><?php 
        echo  __( 'No content wrap', 'wp-data-access' ) ;
        ?>
							</label>
							<br/>
							<input
        ...
        ?>">
					...
					<tr>
						<th><?php 
        echo  __( 'Default database', 'wp-data-access' ) ;
        ?></th>
						<td>
							<div>
								<?php 
        ...
        if ( is_array( $default_databases ) ) {
            foreach ( $default_databases as $user_id => $database ) {
                ?>
										<div id="wpda_default_database_<?php 
                echo  esc_attr( $user_id ) ;
                ?>">
											<span class="dashicons dashicons-trash"
												  style="font-size: 14px; vertical-align: text-top; cursor: pointer;"
												  onclick="if (confirm('Remove default database for this user?')) { jQuery('#wpda_default_database_delete').val('<?php 
                echo  esc_attr( $user_id ) ;
                ?>'); jQuery('#delete_default_user_database_form').submit(); } "
											></span>
											<span>
												<?php 
                echo  esc_attr( $users[$user_id] ) ;
                ?> > <?php 
                echo  esc_attr( $database ) ;
                ?>
											</span>
										</div>
										<?php 
        ...
        ?>
				...
				<div class="wpda-table-settings-button">
					<input type="hidden" name="action" value="save"/>
					<input type="submit" value="<?php 
        echo  __( 'Save Back-end Settings', 'wp-data-access' ) ;
        ?>"
						   class="button button-primary"/>
					<a href="javascript:void(0)"
					   onclick="if (confirm('<?php 
        ...
        ?>')) {
					   ...
					   class="button">
						<?php 
        echo  __( 'Reset Back-end Settings To Defaults', 'wp-data-access' ) ;
        ?>
					</a>
				</div>
				<?php 
        wp_nonce_field( 'wpda-back-end-settings', '_wpnonce', false );
        ?>
			</form>
