...
namespace WPDataAccess {
	...
	class WPDA {
		 ...
		 */
		const OPTION_WPDA_VERSION = [ 'wpda_version', '4.2.7' ];
		/**
		...
		public static function add_wpda_search_args( $columns ) {
			...
			if ( is_array( $columns ) ) {
				...
				foreach ( $columns as $column ) {
					...
					if ( isset( $_REQUEST["wpda_search_column_{$column_name_lwr}"] ) ) {
						if ( is_array( $_REQUEST["wpda_search_column_{$column_name_lwr}"] ) ) {
							// Handle multiple values for same column with OR
							$where_columns_arr = [];
							foreach ( $_REQUEST["wpda_search_column_{$column_name_lwr}"] as $value ) {
							...
							// Handle single value
							$column_date_type = $column['data_type'];
							$column_value     = sanitize_text_field( wp_unslash( urldecode( $_REQUEST["wpda_search_column_{$column_name_lwr}"] ) ) ); // input var okay.
							if ( '' !== $column_value ) {
								if ( 'string' === WPDA::get_type( $column_date_type ) ) {
